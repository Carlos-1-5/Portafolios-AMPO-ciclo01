...
void procesarArchivoCiudades(string nombreArchivo){
	...
	if(!archivo.is_open()){
		cout << "No se pudo abrir el archivo de ciudades proporcionado.\n";
		return;
	}
...
void procesarArchivoVuelos(string nombreArchivo){
	...
	if(!archivo.is_open()){
		cout << "No se pudo abrir el archivo de vuelos proporcionado.\n";
		return;
	}
...
void procesarSolicitudVuelo(){
	...
	if (codigosCiudades.count(ciudadOrigen) == 0){
		cout << "\nLo sentimos, no volamos desde esa ciudad.\n" << endl;
		return;
	}
	...
	if (codigosCiudades.count(ciudadDestino) == 0){
		cout << "\nLo sentimos, no volamos hacia esa ciudad.\n";
		return;
	}
